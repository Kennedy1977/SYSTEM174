...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!snapshot.hasAccessToken && !snapshot.hasRefreshToken) {
    return json({ error: "No SoundCloud tokens are available on the server yet." }, 404);
  }

  return json({
    SOUNDCLOUD_ACCESS_TOKEN: snapshot.accessToken,
    SOUNDCLOUD_REFRESH_TOKEN: snapshot.refreshToken,
    reminder: "Copy these values into your persistent environment, then remove SOUNDCLOUD_TOKEN_EXPORT_KEY from the server.",
  });
};
